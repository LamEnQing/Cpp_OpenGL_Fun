...
namespace OpenGLFun {
	Framebuffer::Framebuffer(int const& pixelsWidth, int const& pixelsHeight) : mTextureId{ 0 }, _bufferId{ 0 } {
	...
	void Framebuffer::Resize(int const& pixelsWidth, int const& pixelsHeight) {
		...
		if (frameBufferError != GL_FRAMEBUFFER_COMPLETE)
			...
			//std::cout << "Framebuffer is not complete, error status:" << frameBufferError;
